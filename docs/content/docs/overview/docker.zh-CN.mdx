...
当前主应用镜像提供静态前端，并把 `/api` 转到公司渠道服务。画布、我的素材和个人 API Key 默认保存在浏览器本地；公司渠道的生图记录和额度存在服务端 `./data` 卷。公司腾讯云点播密钥只放在服务端环境变量，不会下发到浏览器。应用登录用 `ADMIN_USERNAME` / `ADMIN_PASSWORD` 在空库时创建第一个管理员。
