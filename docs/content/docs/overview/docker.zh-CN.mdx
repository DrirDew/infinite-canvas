...
当前主应用镜像提供静态前端，并把 `/api` 转到公司渠道服务。画布、我的素材和个人渠道默认只在当前浏览器里；共享渠道、生图记录和额度存在服务端 `./data` 卷。共享渠道密钥存在服务端环境变量或渠道表；管理员拉取渠道列表时可回显明文，普通用户看不到。应用登录用 `ADMIN_USERNAME` / `ADMIN_PASSWORD` 在空库时创建第一个管理员。
...
| `TENCENT_VOD_SECRET_ID` | 空表时用于创建默认「腾讯云」共享渠道的 SecretId；也可稍后在配置页写入表中 |
| `TENCENT_VOD_SECRET_KEY` | 公司腾讯云 SecretKey；仅管理员可在配置页查看明文 |
