...
不要使用另一套应用的 `.env.production`。修改 `.env` 后执行 `docker compose -f docker-compose.local.yml up -d` 即可让新密码生效，一般不用重新构建镜像。

<Callout type="warn">
不要把真实密码提交进仓库。点播反代路径 `/tencent-vod` 需要使用腾讯云签名头，因此不走这层 Basic 校验；页面本身仍需登录后才能打开。
</Callout>
