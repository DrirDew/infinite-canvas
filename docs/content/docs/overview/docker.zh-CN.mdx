...
当前主应用镜像提供静态前端，并把 `/api` 转到公司渠道服务。画布、我的素材、生成记录和个人 API Key 默认保存在浏览器本地；公司腾讯云点播密钥只放在服务端环境变量，不会下发到浏览器。

## 访问保护（可选）

设置 `AUTH_PASSWORD` 后，打开站点会弹出浏览器自带的用户名密码框。不设置则仍可直接访问。这是 Nginx HTTP Basic 校验，不是账号系统。

把项目根目录的 `.env.example` 复制为 `.env` 后填写：
...
不要使用另一套应用的 `.env.production`。修改 `.env` 后执行 `docker compose -f docker-compose.local.yml up -d` 即可让新密码和公司渠道配置生效，一般不用重新构建镜像。

<Callout type="warn">
不要把真实密码和腾讯云密钥提交进仓库。公司生图走 `/api`，仍受站点 Basic Auth 保护。点播反代路径 `/tencent-vod` 需要使用腾讯云签名头，因此不走这层 Basic 校验，仅留给个人渠道。
</Callout>
