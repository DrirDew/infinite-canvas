...
当前该输入框是原生 `<textarea>`，只能承载纯文本，无法内嵌图片。要显示缩略图，必须改用 `contentEditable`（可编辑 div），把引用做成带 `<img>` 的原子小卡片。

## 关键约束（为什么不能直接改现有组件）

`CanvasResourceMentionTextarea` 被两处复用：

1. **提示词面板** `apps/web/src/components/canvas/canvas-node-prompt-panel.tsx` —— 本次要改的对象。`highlightLabels` 默认 true，value 是提示词字符串，引用以 label 文本「图片1」嵌在其中，**直接发给 AI**（配合 `apps/web/src/lib/image-reference-prompt.ts` 的「参考图片编号：图片1」前缀）。
2. **节点内文本编辑** `apps/web/src/components/canvas/canvas-node.tsx`（`isEditingContent` 分支）—— `highlightLabels={false}`，通过 `ref` 调用 `textarea.setSelectionRange(...)` 等 **textarea 专有 API**。若把组件改成 contentEditable，这处会直接崩。
...
新文件 `apps/web/src/components/canvas/canvas-prompt-chip-input.tsx`，使用 contentEditable 实现图片引用 chip：
...
1. **DOM ← value（未聚焦时重建）**：用 active label 列表（`references.filter(active).map(label)`，按长度降序）把 value 字符串切成「文本片段 + 命中 label」，命中处插入 chip。聚焦时不重建（`document.activeElement === editor` 直接 return），避免与输入 / IME 打架。
...
5. **原子删除**：通过 `deleteAdjacentReference` / `adjacentReferenceNode` / `findReferenceSibling` 处理——contentEditable + `contentEditable="false"` chip 天然整体删除。
...
- 新增：`apps/web/src/components/canvas/canvas-prompt-chip-input.tsx`
- 修改：`apps/web/src/components/canvas/canvas-node-prompt-panel.tsx`（仅替换输入框组件 + 相应 import）
