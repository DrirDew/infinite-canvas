...
To try the project, deploy or start the `apps/web/` frontend.

## Deploy with Vercel

Import the repository into Vercel. The root `vercel.json` builds `apps/web/`. AI requests are sent directly from the browser to your OpenAI-compatible endpoint, so no additional application server is required.
