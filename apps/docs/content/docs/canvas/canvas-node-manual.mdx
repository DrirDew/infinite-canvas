...
To generate an image from text, prepare the prompt and select **Generate Image**. Infinite Canvas creates an image node where you can adjust the model, aspect ratio, and output count before generation.
...
Create a video node from the toolbar or upload a local video. Video nodes use the native player. An empty video node can generate into itself, while generation started from text or images creates a connected video node.
...
5. Adjust parameters and generate from the created image node.
