                ...
                <div className="h-[56vh] min-h-[360px] select-text text-sm" data-canvas-shortcuts-ignore>
                    {view === "info" ? (
                        <div className="thin-scrollbar h-full space-y-3 overflow-auto pr-1">
                            ...
                            <InfoRow label={t("canvas.nodeToolbar.type")} value={isBuiltinNodeType(node.type) ? getNodeSpec(node.type).title : node.type} />
                            ...
                            {batchCount > 1 ? <InfoRow label={t("canvas.nodeToolbar.imageGroup")} value={t("config.image.images", { count: batchCount })} /> : null}
                            {node.metadata?.prompt ? <InfoRow label={t("workbench.prompt")} value={node.metadata.prompt} /> : null}
