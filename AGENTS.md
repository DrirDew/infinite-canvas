...
- 服务端是否存储媒体由配置页「服务端存储」开关控制。开启时保存参考图、参考视频、参考音频和生成结果；关闭时不保存这些媒体文件，只把结果下载链接写入记录。页面展示只看记录里有没有对应数据和文件。
...
- 所有登录用户都可以打开配置页：普通用户可新增/编辑/删除个人渠道、查看共享渠道；只有管理员能开关共享、编辑或删除共享渠道。团队账号 Tab 仍仅管理员可见，用来新增用户、改密码，以及调整每日生图额度和每日生视频额度。额度数字默认只读，点「设置额度」后才可编辑，点「保存」才写入服务端。额度按上海时区每天零点重置已用量。使用统计页不再调整额度；页面与配置页一样用页签，「今日使用」全员可见（普通用户只看自己），「用量统计」仅管理员可见。管理员改用户密码时需验证当前登录管理员的原密码。
- 工作台 `/api` 需要应用登录。`ADMIN_USERNAME` / `ADMIN_PASSWORD` 只用于空库创建第一个管理员，不要写进前端或镜像默认值。
- 共享腾讯云点播生图和生视频由服务端提交任务，完成通知走 `/api/tencent-vod/callback`（腾讯云控制台需配置可公网访问的回调地址）；工作台通过 SSE 更新结果，生成中的刷新按钮由服务端查询腾讯云。个人腾讯云渠道仍由浏览器直连查询。
