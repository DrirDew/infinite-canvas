...
- 外部服务请求统一放在 `web/src/services/api/`。个人渠道仍由浏览器直连第三方接口；共享渠道（当前为腾讯云点播）必须走 `server/`，密钥只放服务端环境变量或 `channels` 表。完整密钥只在管理员拉取渠道列表时返回；普通用户接口不回显密钥，页面用密码框显示掩码且不能睁眼查看。
...
- 共享渠道的生图记录和额度存在服务端（`DATA_DIR`）；个人渠道 API Key 只缓存在当前浏览器会话（`sessionStorage`），关闭浏览器后消失，不要写入 `channels` 表。不要把旧的浏览器生成记录做成迁移。
- 个人渠道默认不入表，由前端直接请求对应接口。管理员在渠道编辑页打开共享开关后才写入 `channels` 表，团队全员可见可用。共享渠道密钥只对管理员接口回显；普通用户看不到明文。管理员导出配置会带上共享渠道和个人渠道，普通用户导出只有个人渠道。空表时创建名为「腾讯云」、协议为腾讯云点播的共享渠道，密钥参照环境变量。
- 所有登录用户都可以打开配置页：普通用户可新增/编辑/删除个人渠道、查看共享渠道；只有管理员能开关共享、编辑或删除共享渠道。团队账号 Tab 仍仅管理员可见，用来新增用户和改密码，不要在该页放额度设置；额度在使用统计页调整。管理员改用户密码时需验证当前登录管理员的原密码。
