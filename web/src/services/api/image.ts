...
export async function requestGeneration(config: AiConfig, prompt: string, options?: RequestOptions) {
    ...
    if (isTencentVodConfig(requestConfig)) {
        ...
        const background = normalizeBackground(config.background);
        try {
            return await requestTencentVodImages(requestConfig, withSystemPrompt(requestConfig, prompt), [], undefined, n, quality, requestSize, background, options?.signal, options?.jobId, options?.wait !== false);
        } catch (error) {
            throw new Error(readAxiosError(error, apiText("requestFailed")));
        }
    }
...
export async function requestEdit(config: AiConfig, prompt: string, references: ReferenceImage[], mask?: ReferenceImage, options?: RequestOptions) {
    ...
    if (isTencentVodConfig(requestConfig)) {
        ...
        try {
            return await requestTencentVodImages(requestConfig, withSystemPrompt(requestConfig, requestPrompt), refs, maskImage, n, quality, requestSize, background, options?.signal, options?.jobId, options?.wait !== false);
        } catch (error) {
            throw new Error(readAxiosError(error, apiText("requestFailed")));
        }
    }
