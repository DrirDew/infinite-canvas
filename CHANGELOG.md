...
+ [调整] 删除画布生成配置节点，文本、图片、视频和音频节点改为直接持有参数并执行生成。
+ [调整] Web 画布改为固定内置节点定义，删除动态节点注册、插件宿主、事件总线和插件私有存储。
+ [调整] 删除旧 Web 节点插件的远程安装、管理、SDK、示例和发布链路，浏览器版不再加载第三方插件代码。
+ [新增] 增加复用现有 Web 应用的 Electron 桌面基础壳和本地开发启动命令。
+ [优化] Core 内部命令按文档、选择、节点交互、连线、剪贴板和历史拆分，并新增集中式 `policies` 与 `listeners` 接入配置。
+ [优化] Core 原子修改与预览提交统一校验最终文档，临时交互改为互斥的 `kind` 状态并保留原有便捷字段。
...
+ [优化] Core 连线层支持按可见节点裁剪 SVG 路径，Web 千节点画布不再挂载完全离屏的普通连线。
+ [调整] Web 画布工具、Agent 桥接与插件读取 API 跟随 Core 使用只读节点和连线快照，避免宿主绕过命令原地修改文档。
+ [优化] Core 将节点拖动、框选和连线移动统一合并为每动画帧一次，并在指针抬起时提交最终坐标，降低高频交互重渲染。
+ [优化] Core 为大型不可变节点快照增加弱缓存空间索引，降低千节点画布的视口裁剪、框选和连线命中扫描成本。
+ [优化] Core 画布文档改为只读快照契约，外部数据进入实例时自动断开结构引用，并拒绝非有限坐标和非正节点尺寸。
+ [优化] Core 明确区分实例初始化参数与运行时命令，并减少连线命中和关系遍历中的重复查找。
+ [修复] Web、Docs 与 Example 的独立 TypeScript 类型检查恢复通过，构建流程仍不捆绑类型检查。
+ [调整] Core npm 包名改为 `@basketikun/infinite-canvas`，其余工作区包统一使用 `@basketikun` scope，并同步更新内部依赖、示例与文档引用。
+ [调整] Core Example 改为接近官方 Web 结构的单画布演示，并明确基础交互与宿主节点内容的职责边界。
+ [修复] Core 画布支持在项目数据恢复后延迟挂载容器，避免可视区域保持零尺寸导致节点不可见。
+ [修复] Core 交互 Hook 修复外部回调与内部稳定处理器重名导致的 Example 启动失败。
+ [调整] 画布视口、基础类型、主题和几何能力迁入可独立发布的 Core，并新增独立 Example 应用。
+ [调整] 画布文档、选择与撤销重做改由 Core 实例管理，Web 项目仅持久化文档快照并通过 Core 命令修改。
+ [调整] Web 画布移除本地选择及节点、连线数组适配，媒体导入、生成与多图更新改为直接调用 Core 命令。
+ [调整] Web 画布、工作台与 Agent 统一使用 Core 主题定义，并删除重复的 Web 主题文件。
+ [调整] 容器尺寸、坐标转换、框选、节点选择与拖动、连线移动及全局指针生命周期迁入 Core，Web 仅处理交互结果对应的业务动作。
+ [调整] 画布复位、中心缩放与节点聚焦动画改由 Core 实例控制，Web 不再维护视口动画状态。
+ [调整] 节点缩放和连线起点事件改由 Core 交互入口直接转为实例命令，减少 Web 画布事件适配。
+ [修复] 手动平移、滚轮缩放或操作小地图时会由 Core 取消未完成的节点聚焦动画，避免视口继续回弹。
+ [调整] 连线点击与右键选择改由 Core 交互入口处理，独立示例补充连线选择和删除。
+ [修复] 节点缩放在指针取消、窗口失焦或控制点卸载时会正确结束，避免画布残留缩放状态。
+ [修复] 画布平移中窗口失焦会丢弃尚未提交的视口帧，避免取消后继续发生位移。
+ [调整] Core 支持注入连线解析策略，官方 Config 节点规则移回 Web 适配层，独立应用不再继承业务节点语义。
+ [调整] Core 节点类型改为应用自定义字符串，分组节点通过独立角色标记，官方媒体与配置类型不再属于 Core API。
+ [调整] Core 节点分组归属改为顶层字段，业务 metadata 泛型不再被引擎字段约束。
+ [新增] Core 画布实例支持配置历史容量、拖动阈值、分组边距和连线命中范围。
+ [新增] Core 支持配置缩放范围、节点聚焦动画参数与背景网格间距。
+ [新增] Core 支持注入节点分组策略，接入应用可限制允许吸附和归组的节点组合。
+ [新增] Core 新增 headless 与 react 独立包入口，支持按运行环境引用画布能力。
+ [优化] Core headless 坐标与快捷键 API 移除浏览器 DOM 类型依赖，纯 Node TypeScript 项目可直接引用声明。
+ [调整] Core 画布、节点与连线起点统一使用 Pointer Events 和一致的 PointerDown API。
+ [新增] Core 支持订阅选择与临时交互状态变化，方便接入外部检查器和状态面板。
+ [修复] Core 选择命令会过滤文档中不存在的节点和连线，避免产生悬空选择状态。
+ [修复] 节点拖动、缩放和连线创建仅响应主键指针，右键菜单不再触发编辑状态。
+ [修复] Core 支持取消未提交预览，撤销重做与事务不再丢失预览起点。
+ [调整] Core 将手动视口输入回调明确为 onViewportInput，区分渲染入口与全量状态回调。
+ [新增] Core 提供 useCanvasEditor 高层 Hook，可一次组合画布状态与交互编排。
+ [修复] Core 新增节点和连线命令会过滤重复 ID、无效端点及策略拒绝的连接结果。
+ [新增] Core 提供外部画布文档结构校验 API，可在加载前报告无效节点、分组和连线。
+ [新增] Core 连线层支持注入自定义路径解析器。
+ [修复] Core 节点更新会阻止重复 ID，并同步清理角色或类型变化产生的无效分组和连线。
+ [修复] Core 节点缩放中断时会回滚未提交预览，不再把失焦或卸载误记为一次编辑。
+ [新增] Core 连线层支持定制连线样式、活动预览样式和命中宽度。
+ [新增] Core 框选层支持覆盖容器样式和矩形 SVG 属性。
+ [新增] Core 小地图支持配置世界边距、最小可见尺寸和容器类名。
+ [优化] Core npm 包补齐仓库、文档、问题反馈和随包许可证元数据。
+ [优化] Core npm 包移除未使用的 react-dom peer dependency，headless 与自定义宿主接入只需按实际能力安装依赖。
+ [修复] Core 替换画布文档时会同步触发文档快照回调并重置实例历史。
+ [修复] Core 小地图在空文档时也会应用配置的世界边距。
+ [修复] Core 多实例画布的键盘工具切换、平移监听和抓取光标不再互相干扰。
+ [修复] Core 平移被 pointercancel 中断后会丢弃未提交视口帧，避免取消后继续位移。
+ [优化] Core 多实例交互共享全局窗口事件监听，避免重复注册相同原生事件。
+ [修复] Core 取消框选或卸载交互 Hook 时会释放指针所有权，并清理未提交拖动与连线状态。
+ [修复] Core 粘贴会复用节点与连线校验，阻止冲突 ID 和无效分组进入文档。
+ [修复] Core 分组策略统一约束拖拽、文档命令、粘贴和外部文档校验。
+ [调整] Core 对外节点选择集合改为只读类型，选择修改统一通过实例命令完成。
+ [新增] Core 节点缩放角和连线端口支持自定义尺寸、偏移与渲染内容。
+ [优化] Core 发布产物改为 NodeNext ESM，可被原生 Node 与现代 bundler 直接解析。
+ [修复] Core 主题类型允许第三方使用任意 CSS 颜色，不再被默认色值字面量限制。
+ [新增] Core 小地图支持自定义节点内容、节点样式和视口样式。
+ [修复] Core 小地图导航不再同时触发主画布框选或滚轮缩放。
+ [新增] Core 小地图支持可访问名称、焦点入口和可配置方向键导航。
+ [修复] Core 节点等值更新不再发布重复快照或产生空撤销历史。
+ [优化] Core 重复选择和等值视口写入不再触发多余状态与回调更新。
+ [新增] Core 公开节点控制、连线命中和小地图渲染默认参数。
+ [修复] Core 多指针交互按启动 pointerId 隔离，额外触点不再结束当前操作。
+ [修复] Core 节点缩放会忽略第二触点的开始、抬起与取消事件，避免提前提交或回滚当前缩放。
+ [修复] Core 同一画布内的平移、框选、节点拖动和连线共享指针所有权，额外触点不再并行启动其他交互。
+ [调整] Core 查询、几何、渲染和批量新增接口支持只读节点与连线数组。
+ [新增] Core 连线层支持自定义 SVG 容器类名与样式。
+ [新增] Core 连线支持键盘焦点、可访问名称、选中状态和 Enter/Space 选择。
+ [新增] Core 画布支持自定义背景渲染与背景容器样式。
+ [新增] Core 世界内容层支持自定义类名与样式，基础渲染层补充稳定语义标记。
+ [调整] Core 节点缩放支持自定义最小宽高，官方 Web 继续使用原有最小尺寸。
+ [调整] 节点生成状态类型移回 Web，Core 公共文档类型不再包含业务加载状态。
+ [修复] Agent 创建连线时复用 Core 与官方节点规则，阻止分组或无效 Config 连线。
+ [修复] Agent 删除分组节点时复用 Core 文档规则，关联连线和子节点分组关系会同步清理。
+ [修复] Core 连线渲染移除固定世界尺寸限制，负坐标和远距离节点之间的连线可正常显示与选择。
+ [调整] Core 独立示例补充文档快照回调与未知节点占位展示，覆盖计划中的完整多实例验证场景。
+ [调整] 画布视口状态、坐标转换与框选命中迁入 Core，Web 仅保留视口持久化和临时指针交互状态。
+ [调整] 节点拖动、缩放、分组与吸附的预览和历史提交迁入 Core，Web 改为通过稳定命令转发交互。
+ [调整] 连线目标命中、预览与结束状态迁入 Core，Web 仅负责连接 ID、持久化和空白处业务创建菜单。
+ [调整] 画布复制粘贴、分组与连线重映射及跨平台快捷键识别迁入 Core，系统剪贴板媒体导入继续由 Web 处理。
+ [调整] 节点定位外壳、缩放控制点、连接端口、未知节点占位、连线层、框选矩形和小地图迁入 Core，并删除 Web 对应重复组件。
