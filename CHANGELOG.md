...
+ [修复] 腾讯云点播 AIGC 完成回调兼容 `AigcImageTaskCompleteEvent` / `AigcVideoTaskCompleteEvent` 字段，生成结果可自动从「生成中」更新，无需手动刷新。
+ [调整] 生图和生视频改为各自的每日额度，按上海时区每天零点重置已用量；管理员在团队账号页调整额度，使用统计页不再改额度。
+ [调整] 使用统计拆为「今日使用」和「用量统计」：今日使用列出各用户当日剩余额度并可查看当天详情；用量统计仅管理员可见，按模型、渠道展示生图张数和生视频时长。
+ [调整] 团队账号需先点「设置额度」再编辑每日额度，点「保存」后才写入服务端。
+ [修复] 今日使用查看详情在当天已有成功记录时不再因用量统计接口报 500。
+ [优化] 团队账号与今日使用的生图/生视频额度改为同一套右对齐数字排版。
+ [调整] 用量统计占比表增加渠道列，按渠道和模型分别计算占比。
+ [调整] 生图和视频工作台生成记录里「生成中」状态改为黄色标签。
+ [调整] 用量统计折线图图例居中，悬停时用跟随鼠标的浮层显示该时间点数值。
+ [调整] 本地 Docker 服务改为 server / client，Nginx 反代到 server。

+ [新增] 腾讯云点播渠道支持生视频，可选可灵、Vidu、海螺、混元、明眸、GV、OS、PixVerse 等模型；共享渠道与生图一样走回调和 SSE，生成中可手动刷新。
+ [调整] 共享腾讯云点播生图改为提交任务后由回调通知完成，工作台通过 SSE 更新结果；生成中可在结果右侧手动刷新（间隔 5 秒），由服务端向腾讯云查询状态。

+ [新增] 配置页「服务端存储」可开关是否把参考媒体和生成结果写入服务端；关闭时只保留结果下载链接，页面按记录里实际有的数据展示。
+ [新增] 生图工作台和视频创作台点「新建」会立即写入生成记录，可保存草稿，开始生成和任务结束会更新同一条记录。
+ [调整] 草稿点开始生成会立刻变为「生成中」；已完成记录再生成会立即新建一条「生成中」记录。记录会保存更新时间、开始生成时间和任务结束时间。
+ [调整] 共享渠道改存服务端 `channels` 表；新增默认是个人渠道，只缓存在当前浏览器会话。空表时创建名为「腾讯云」的共享渠道，密钥参照环境变量。
+ [调整] 所有登录用户可打开配置页：普通用户管理个人渠道并查看共享渠道；只有管理员能在编辑页开关共享，或删除共享渠道。
+ [调整] 去掉渠道「设为默认」。敏感配置默认闭眼显示掩码：管理员可睁眼查看明文，普通用户不能。管理员导出会包含共享和个人渠道，普通用户只导出个人渠道。
+ [新增] 公司渠道生图会按成功张数扣除额度，记录和图片保存在服务端；额度不够时不会请求腾讯云。
+ [新增] 增加使用统计页：可查看剩余额度和用量流水，管理员可在该页给同事加额度。
+ [调整] 团队账号改为与渠道类似的列表和抽屉：新增用户需重复密码，管理员可修改所有用户密码，可删除普通用户；管理员账号删除图标为灰色且不能删除。
+ [调整] 生图工作台和视频创作台选择模型改为先选渠道、再选模型。
