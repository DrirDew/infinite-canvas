...
实例返回的 `selectedNodeIds`、`CanvasSelection.nodeIds` 和框选结果使用 `ReadonlySet`，接入应用通过选择命令修改状态，不直接写入 Core 内部集合。重复选择和相同视口值不会触发 React 状态或外部回调；`setDocument` 仍会强制发布实例重置。节点更新会比较最终引擎字段与 metadata 引用，语义无变化的 patch 不发布快照或增加历史。
