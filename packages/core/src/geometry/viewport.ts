...
/** Converts a canvas world coordinate into a browser client coordinate. */
...
/** Creates a viewport whose world origin is centered in the container. */
export function centerViewport(size: CanvasSize, k = 1): ViewportTransform {
    return { x: size.width / 2, y: size.height / 2, k };
}

/** Zooms around the center of a viewport container. */
...
/** Zooms around a point expressed in container coordinates. */
...
/** Computes a viewport that centers and fits one node within the requested coverage. */
...
/** Returns nodes intersecting the visible viewport plus optional world-space padding. */
