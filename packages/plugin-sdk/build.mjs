...
// 产物同步到 apps/web/public/plugins),消除各插件重复的构建脚本。
...
export async function buildPlugin(metaUrl, overrides = {}) {
    ...
    // 默认同步到仓库内 apps/web/public/plugins(plugins/user/<name> → 上溯三层到仓库根)
    const publicDir = overrides.publicDir ?? join(root, "..", "..", "..", "apps", "web", "public", "plugins");
    ...
    const syncToPublic = {
        ...
        setup(builder) {
            builder.onEnd(async (result) => {
                ...
                console.log(`[${name}] synced → apps/web/public/plugins/${name}.js`);
            });
        },
    };
    ...
    const options = {
        ...
        jsx: "automatic",
        jsxImportSource: "@basketikun/plugin-sdk",
