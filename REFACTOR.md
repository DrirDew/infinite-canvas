...
- Core 公开纯文档操作，Agent 删除节点与连线复用同一实现，删除分组时同步解除子节点关系。
- Web 的分组子节点计数和关联高亮查询迁入 Core selectors。
- Web 插件与资源适配层的上下游节点查询复用 Core selectors，业务节点筛选仍留在 Web。
- Core 连线 SVG 使用可溢出的世界坐标层，不再用固定 `10000×10000` 尺寸限制负坐标或远距离节点。
- AI 重试的上游节点遍历复用 Core 图查询，具体 Config 类型判断继续由 Web 注入。
- Core 将 `groupId` 提升为节点结构字段并解除 metadata 泛型约束，第三方可使用任意业务元数据类型。
- `useCanvas` 开放历史容量、拖动阈值、分组边距与连线命中范围配置，默认参数由 Core 统一导出。
...
- Headless 坐标与快捷键 API 使用 Core 自有结构类型，纯 Node TypeScript 消费者无需启用 DOM lib。
- Core 提供独立 headless 类型契约配置，以纯 ES lib 和空全局 types 验证 Node 消费者入口。
...
- 空文档与有节点文档的小地图统一使用 `worldPadding`，首次新增节点前后不再切换到固定世界范围。
...
- `useCanvas` 缩减为 React 状态与最新引用组合层，历史、预览、拖动、缩放、连线和剪贴板命令由内部命令工厂集中构造，公开命令对象仍保持稳定。
- 命令工厂进一步将 React 状态发布、选择清理、历史和预览事务抽成独立运行时，领域命令只负责画布行为组合。
- 新增节点与粘贴统一清理无效分组归属；粘贴生成的空、重复或冲突 ID 不再绕过节点和连线新增校验。
- `canGroupNode` 统一应用于拖拽、新增、更新、缩放、粘贴和外部文档校验，直接写入 `groupId` 不再绕过宿主策略。
- Core 对外暴露的节点选择集合统一为 `ReadonlySet`，宿主只能通过选择命令触发状态与回调更新。
- 节点缩放角与连线端口开放命中尺寸、偏移和 `renderHandle`，宿主可保留 Core 指针逻辑并替换控制点视觉。
- Core 发布构建切换到 NodeNext ESM，所有内部模块使用显式 `.js` 引用，并输出源码映射与声明映射供 Node 和 bundler 消费。
- `CanvasTheme` 改为可由宿主实现的结构类型，不再把第三方颜色限制为官方明暗主题的字面量。
- `CanvasMinimap` 在保留 Core 布局和导航逻辑的同时开放节点内容、节点样式和视口样式扩展。
- `CanvasMinimap` 提供可访问名称、焦点入口和可配置方向键步长，指针与键盘都可独立导航视口。
- 节点更新在策略清理后比较最终节点与连线引用，等值 patch、等值位置和被拒绝后恢复原值的分组更新不再产生空历史。
- 重复节点/连线选择和等值视口写入不再触发状态或回调；`setDocument` 继续强制发布选择与交互重置。
- 节点最小尺寸、缩放控制点、连线端口与命中层、小地图尺寸和世界边距等渲染默认值集中到公开 `canvasDefaults`。
- 平移、框选、节点拖动、连线和小地图导航绑定启动操作的 `pointerId`，额外触点不会移动或结束其他指针的交互。
- 几何、选择器、基础渲染和批量新增 API 接受 readonly 节点/连线数组，宿主不可变状态无需在调用 Core 前复制。
- `CanvasConnectionLayer` 开放 SVG 容器类名与样式，连线、框选和小地图基础层拥有一致的宿主布局入口。
- `CanvasConnectionLayer` 的持久连线命中路径支持焦点、可访问名称、按压状态和 Enter/Space 键盘选择。
- `InfiniteCanvas` 开放背景容器样式和自定义背景渲染，宿主可读取当前视口、主题、模式与网格尺寸，并继续复用 Core 的点击穿透层。
- `InfiniteCanvas` 开放世界内容层类名与样式，基础渲染层提供稳定语义 data 标记供宿主 CSS、测试和调试使用。
- Core npm 包补齐仓库、文档、问题反馈、关键词与随包 MIT 许可证元数据。
- Core README 按安装入口、模型命令、策略回调、交互、渲染扩展、边界和源码职责组织，第三方无需阅读 Web 代码即可接入。
