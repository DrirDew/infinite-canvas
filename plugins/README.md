...
```
packages/plugin-sdk/ # 插件 SDK(类型 + automatic JSX 运行时 + 构建助手)
plugins/user/
...
```bash
cp -r plugins/user/template plugins/user/my-plugin
cd plugins/user/my-plugin
# 改 package.json 的 name;改 src/index.tsx 里的 id / nodes[].type
bun install
bun run dev        # watch 构建,产物同步到 apps/web/public/plugins/my-plugin.js
bun run typecheck  # tsc --noEmit,类型自检
```
...
```bash
cd plugins/user/<name>
bun run build   # → dist/<name>.js,并同步到 apps/web/public/plugins/<name>.js
bun run dev     # watch,改动自动构建并同步
```
...
`bun run dev` 起 watch,产物会同步到 `apps/web/public/plugins/<name>.js`。此后有两种方式在画布里用到它:

**方式一(推荐):自动发现。** 画布启动时会扫描 `apps/web/public/plugins/` 下的插件,自动加入「节点插件」管理器列表,**默认关闭**;打开开关即启用。无需手动填 URL,启用时会按文件重新拉取,配合 watch 改完刷新即最新。

**方式二:`VITE_DEV_PLUGINS`。** 在 `apps/web/.env.local` 声明(逗号分隔多个),这些插件每次刷新页面都**重新拉取并直接激活**(不缓存、不落库、无开关):

```env
VITE_DEV_PLUGINS=/plugins/markdown.js,/plugins/svg.js
```

再从仓库根目录执行 `bun run dev`。流程即:改 `src/index.tsx` → watch 自动构建 → 刷新画布看到最新效果,无需反复安装。
...
> `metadata` 的**内置字段**(content、status、model…)是强类型;插件写入的**自定义字段**读出为 `unknown`,按需 `as` 断言。
